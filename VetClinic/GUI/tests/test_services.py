...
def test_invoice_list_by_client(mock_list_invoices, mock_Session):
    class DummyInv:
        def __init__(self, id, client_id): self.id = id; self.client_id = client_id
...
def test_authservice_login(mock_post):
    ...
    s = AuthService("http://fakeurl")
    resp = s.login("e@x.pl", "haslo", otp_code="111", totp_code="222")
    assert mock_post.called
    # Sprawdź czy payload się składa
...
def test_authservice_confirm_totp(mock_post):
    from vetclinic_gui.services.auth_service import AuthService
    s = AuthService()
    s.confirm_totp("e@x.pl", "123456")
    mock_post.assert_called_with(
        f"{s.base}/users/confirm-totp",
...
def test_authservice_change_password(mock_post):
    from vetclinic_gui.services.auth_service import AuthService
    s = AuthService()
    s.change_password("e@x.pl", "old", "new", reset_totp=True)
...
def test_authservice_setup_totp(mock_post):
    ...
    s = AuthService()
    s.setup_totp("e@x.pl")
    # sprawdź czy przekazany email przez params
    assert mock_post.call_args[1]['params'] == {"email": "e@x.pl"}


# ================== BlockchainService ==================

@patch("vetclinic_gui.services.blockchain_service.Web3")
@patch("vetclinic_gui.services.blockchain_service.settings")
def test_blockchainservice_init_and_no_connection(mock_settings, mock_Web3):
    # Brak połączenia
    ...
    s = BlockchainService(provider_url="http://127.0.0.1:99", contract_address="0x123", abi_path="missing.json")
    assert not s.connected
    # Sprawdza, czy wyświetla warning gdy brak ABI
    # Brak pliku ABI lub adresu = warning — możesz sprawdzić, że .contract is None
...
@patch("json.load")
@patch("vetclinic_gui.services.blockchain_service.settings")
def test_blockchainservice_init_contract_success(mock_settings, mock_json, mock_open, mock_Web3):
    # Udane wczytanie kontraktu
    mock_Web3.return_value.is_connected.return_value = True
    mock_Web3.to_checksum_address.side_effect = lambda x: x
    mock_json.return_value = [{"abi": "def"}]
    mock_contract = MagicMock()
    mock_Web3.return_value.eth.contract.return_value = mock_contract
    mock_Web3.return_value.eth.accounts = ["0x111"]
    from vetclinic_gui.services.blockchain_service import BlockchainService
    s = BlockchainService(provider_url="http://abc", contract_address="0x123", abi_path="test.json")
    assert s.contract is not None
    assert s.service_account == "0x111"
...
def test_animalservice_list_by_owner():
    from vetclinic_gui.services.animals_service import AnimalService
    class Dummy:
        def __init__(self, owner_id): self.owner_id = owner_id
    # patchuje .list
...
def test_appointments_get_free_slots(mock_session):
    ...
    mock_session.return_value = db
    from vetclinic_gui.services.appointments_service import AppointmentService
    # Sprawdzamy dzień roboczy
    slots = AppointmentService.get_free_slots(1, "2025-06-30")
    assert "08:00" in slots
    # Sprawdzamy niedzielę (brak slotów)
    slots2 = AppointmentService.get_free_slots(1, "2025-06-29")
    assert slots2 == []
    # Sprawdzamy zły format daty
    slots3 = AppointmentService.get_free_slots(1, "ZŁY")
    assert slots3 == []
